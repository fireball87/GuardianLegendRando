...
class Generator
{
    ...
    /* an old abandoned version of subdevide, will probably delete soon
     *
     * private function subdivide(Map $map)
    {
        //decide on a division strategy
        //strategy 0 is opposite corners
        //strategy 1 is opposite edges
        //strategy 2 and 3 are one corner one opposing edge for weighting to prefer this, this is the base games strategy
        //we force opposing edges because this subdivision takes space from other areas

        $strategy = rand(0,3);

        //decide which corner/edge to start our division with
        $base = NULL;

        do {
            $base = rand (0,4);;
        } while ($base != 2); //this could roll a bunch of 2s and waste time, but it keeps the code easy

        // 0 1 2
        // 3 x 5
        // 6 7 8

        $division_point0 = NULL;
        $division_point1 = NULL;
        switch ($strategy) {
            case 0:
                $division_point0 = $base * 2;
                $division_point1 = 8 - $division_point1;
                break;
            case 1:
                $division_point0 =  $base * 2 - 1;
                $division_point1 = 8 - $division_point1;
                break;
            default:
                $division_point0 = $base * 2;
        ...
        // now we have 2 division points $division_point0 and $division_point1
        //we are still using this area coding
        // 0 1 2
        // 3 4 5
        // 6 7 8

        //lets start by filling a basic grid
        $mapdata = &$map->data;
        foreach ($mapdata as $yPos => &$row) {
            foreach($row as $xPos => &$item) {
                if($xPos < 8)
                {
                    if($yPos < 8)
                    ...
                    elseif($yPos >= 8 && $yPos < 16)
                    {
                        $item->area = 3;
                    }
                    elseif($yPos >= 16 && $yPos < 24)
                    {
                        $item->area = 6;
                    }
                }
        ...
        $this->split_edge(7, 9, $map);

    }

    private function split_edge(int $edge, int $next_edge, Map &$map)
    {
        //for now we'll split edges with only one strategy, that being to cut them directly in half
        $start_x1 = NULL;
        $start_y1 = NULL;
        $end_x1 = NULL;
        $end_y1 = NULL;
        ...
        //$this->block_fill_area($map,$start_y1,$start_x1,$end_y1,$end_x1,$edge);
        $this->block_fill_area($map,$start_y1,$start_x1,$end_y1,$end_x1,$edge);
        $this->block_fill_area($map,$start_y2,$start_x2,$end_y2,$end_x2,$next_edge);

    }

    private function block_fill_area(Map &$map, int $start_y, int $start_x, int $end_y, int $end_x, int $value)
    {
        for ($y = $start_y; $y <= $end_y; $y++) {
            for ($x = $start_x; $x <= $end_x; $x++) {
                $map->data[$y][$x]->area = $value;
            }
        }
    }*/
}